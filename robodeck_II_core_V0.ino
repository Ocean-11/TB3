...
void setup()
{
  ...
  else
  {
    ...
    DEBUG_SERIAL.println("Dynamixel is moving...");

    dxl_wb.goalPosition(dxl_id, (int32_t)goal_servo_angle); 
    delay(1000);
    dxl_wb.goalPosition(dxl_id, (int32_t)(goal_servo_angle+150));
    delay(1000);
    dxl_wb.goalPosition(dxl_id, (int32_t)goal_servo_angle);
  }
  */
...
void servoCallback(const std_msgs::Int32& servo_angle_msg)
{
  ...
  goal_servo_angle = angle_request % (MAX_POSITION + 1);  
  sprintf(log_msg, "servoCallback: setting goal to %i", goal_servo_angle);
  nh.loginfo(log_msg);
}
...
void payloadModeCallback(const std_msgs::Bool& pld_auto_msg)
{
  ...
  if (pld_auto_mode)
  {
    ...
    goal_servo_angle = HVLP_CLOSE_POS;    
    sprintf(log_msg, "Payload state = auto, HVLP shut OFF");
  }
  else
  {
    // switch payload auto mode to ON
...
void setServoAngle(int32_t servoAngle)
{
  char log_msg[50];    
  //dxl_wb.goalPosition(DXL_ID, (int32_t)servoAngle);   
  myservo.write(servoAngle);  
  current_servo_angle = servoAngle;  
  sprintf(log_msg, "Setting HVLP servo angle to %i", servoAngle);  
  nh.loginfo(log_msg);
}
...
void payloadHandler(void)
{
  ...
  actual_angle_goal = HVLP_CLOSE_POS;
  ...
  if (payload_auto_mode)
  {
    ...
    {        
      // Forward -> Set servo angle goal to OPEN
      actual_angle_goal = HVLP_OPEN_POS;                    
    }
    else
    {
      // otherwise -> shut down HVLP sparyer       
      actual_angle_goal = HVLP_CLOSE_POS;       
    }           
  }
  else
  {
    // Handle Manual mode 
    // ------------------      
    actual_angle_goal = goal_servo_angle;      
  }
   
  // set servo angle to goal   
  if (current_servo_angle != actual_angle_goal)
  {
    sprintf(log_msg, "Auto: Set servo to %i",  actual_angle_goal);
    nh.loginfo(log_msg);    
    setServoAngle(actual_angle_goal);     
  } 
}
